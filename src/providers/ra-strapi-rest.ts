...
import {resourceType} from "../constants/api";

type requestType = 'GET_LIST' | 'GET_ONE' | 'GET_MANY' | 'UPDATE' | 'UPDATE_MANY' | 'CREATE' | 'DELETE' | 'DELETE_MANY' | 'GET_MANY_REFERENCE' | string
interface RequestOptions {
    method?: 'PUT' | 'POST' | 'DELETE' | 'GET' | 'OPTIONS';
    body?: string
}
...
export default (apiUrl:string, httpClient = fetchUtils.fetchJson, uploadFields = []) => {
     ...
     */
    const convertDataRequestToHTTP = (type: requestType, resource: resourceType | string, params: RequestParams) => {
        let url = '';
        ...
        switch (type) {
            ...
            case UPDATE:
                ...
                const {created_at, updated_at, createdAt, updatedAt, publishedAt, ...data} = params.data;
    ...
    const adjustQueryForStrapi = (params: RequestParams) => {
    ...
    const handleFileUpload = (type: requestType, resource: resourceType, params: RequestParams, uploadFieldNames: any[]) => {
    ...
    const replaceRefObjectsWithIds = (json: Record<string, any>) => {
     ...
     */
    const convertHTTPResponse = (response: ResponseType, type: requestType, resource: resourceType, params: RequestParams) => {
        const { headers, json } = response;
        switch (type) {
            case GET_ONE:
                return { data: replaceRefObjectsWithIds(convertResponseObject(json?.data) || json) };
            case GET_LIST:
            case GET_MANY_REFERENCE:
                return {
                    data: json.data ? json.data.map((item: Record<string, any>) => convertResponseObject(item)) : json,
                    total: json?.meta?.pagination?.total || 1
                };
            case UPDATE:
                return { data: convertResponseObject(json?.data) };
            case CREATE:
                return { data: convertResponseObject(json?.data) };
            case DELETE:
                return { data: { id: null } };
    ...
    /**
     * most response data objects hasn't plain structure, but has nested object attributes in his structure like {id: i, attributes: { ... }}
     * this converter hoists attributes object to top level
     * @param data {id: i, attributes: { ... }}
     */
    const convertResponseObject = (data: Record<string, any>) => {
     ...
     */
    return (type: requestType, resource: resourceType, params: RequestParams) => {
